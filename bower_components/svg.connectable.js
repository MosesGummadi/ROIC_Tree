...
(function e(t, n, r) {
    function s(o, u) {
        ...
        function connectable(options, elmTarget) {
            ...
            elmTarget.sources = elmTarget.sources || [];
            elmTarget.sources.push(con);
            ...
            con.remove = function () {
                for (var i = 0; i < elmSource.cons.length; i++) {
                    // console.log(_connections);
                    // console.log(_betweenTwoBubbles);
                    if (elmSource.cons[i].id == con.id) {
                        con.marker.remove();
                        con.line.remove();
                        delete _betweenTwoBubbles[con.id];
                        elmSource.cons.splice(i, 1);
                    }
                }
            }
